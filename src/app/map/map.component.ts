...
export class MapComponent implements OnInit {
  ...
  toggleLatest(){
    this.showLatestBloc = ! this.showLatestBloc;
  }
  ...
  searchFor(e) {
    if (e.target.value != "") {
      this.database.search().subscribe((infections) => {
        this.seachInfections = [];
        infections.forEach(infect => {


          if (
            infect.data().userinformations.fullname.indexOf(e.target.value.toLowerCase().trim()) != -1
            ||
            ( (infect.data().userinformations.phone.indexOf(e.target.value.toLowerCase().trim()) != -1) && (e.target.value.trim().length ==8 ) )

          ) {
            this.seachInfections.push(
              {
                id: infect.id,
                data: infect.data()
              }
